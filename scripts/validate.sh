...
if rg -n '<(TODO|TBD|FIXME|fill-|replace-|your-)[^>]*>' examples; then
  echo "Unfilled placeholder in generated example" >&2
  exit 1
fi

if rg -n 'BEGIN [A-Z ]+ KEY|AKIA[0-9A-Z]{16}|gh[pousr]_[A-Za-z0-9]{20,}|sk-[A-Za-z0-9]{20,}' . --glob '!LICENSE' --glob '!.git/**'; then
...
for blueprint in \
  ...
  rg -q "blueprints/${blueprint}" README.md || { echo "Blueprint missing from README: $blueprint" >&2; exit 1; }
  rg -q "blueprints/${blueprint}" wizard.md || { echo "Blueprint missing from wizard: $blueprint" >&2; exit 1; }
  rg -q '../../rules.md' "blueprints/${blueprint}" || { echo "Blueprint missing central-rules reference: $blueprint" >&2; exit 1; }
done

if rg -n '\]\(\.\./rules\.md\)' blueprints; then
  echo "Stale blueprint rules link found" >&2
  exit 1
fi

rg -q '^# Central Rules$' rules.md || { echo "rules.md is missing its central section" >&2; exit 1; }
if rg -l '^# Central Rules$' --glob '*.md' | grep -v '^rules.md$' | grep -q .; then
