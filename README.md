...
Este projeto não é uma CLI funcional, aplicação executável, API, integração externa ou automação de publicação. O `wizard.md` é um guia conversacional em Markdown que pode ser seguido por diferentes agentes de código.
...
O wizard não deve ser tratado como uma CLI. O usuário precisa responder ao diagnóstico, revisar o plano e confirmar gravações. O agente pode analisar documentos, propor artefatos e gerar arquivos autorizados; não deve copiar o kit inteiro sem necessidade.
...
## Estrutura de um workspace gerado
